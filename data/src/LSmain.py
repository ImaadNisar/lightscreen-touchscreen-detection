...
def warpImage(cap):
    ...
    pts2 = np.float32([[0, 0], [500, 0], [0, 500], [500, 500]])  # creates destination matrix of points on warped frame
    matrix = cv2.getPerspectiveTransform(pts1, pts2)  # creates perspective transform matrix for warping below
    
    maskImage(cap, matrix)
...
def maskImage(cap, mat):
    ...
    while True:
        ...
        frame = cv2.warpPerspective(frame, mat, (500, 500))
        ...
        trackLED(maskparams, frame, hsvimg)
...
def manualMaskParams(img, hsv):
    ...
    for i in range(1, 7):
        pos.append(cv2.getTrackbarPos(tbn[i], n))
    ...
    if cv2.waitKey(1) == 13:
        cv2.destroyWindow(n)
        return [lower, upper, "saved"]

    return [lower, upper]


def automaticMaskParams(img, hsv):
...
def trackLED(maskparams, frame, hsv):
